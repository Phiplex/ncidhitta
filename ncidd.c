...
int main(int argc, char *argv[])
{
    ...
    cpid = getpid();                                                                                          /* LA: Added */
    ppid = getppid();                                                                                         /* LA: Added */  
    openlog("ncidd", LOG_PID | LOG_NDELAY | LOG_CONS, LOG_USER);                                              /* LA: Added */
    syslog(LOG_NOTICE, "NCID Daemon version %s PID=%d & Parent PID=%d Starting...", VERSION, cpid, ppid);     /* LA: Added */

    logMsg(LEVEL1, "=====================================================================\n");                /* LA: Added */
    
    sprintf(msgbuf, "Started: PID=%d & Parent PID=%d \nServer: %s %s\n%s\n", cpid, ppid, name, VERSION, API); /* LA: Changed */
...
int doTTY()
{
    ...
    if (!nomodem)
    {
        ...
        /* LA: arguement checkmodem = 1 (true) added to doModem() */
        /* if (doModem() < 0) errorExit(-1, ttyport, 0);          */
        if (doModem(1) < 0) errorExit(-1, ttyport, 0);
    }
...
/* LA: argument (int checkmodem) added */
/* int doModem()                 */
 int doModem(int checkmodem)
{
    int cnt, ret = 2;
    char msgbuf[BUFSIZ];
    ...
    /* LA: fixModembuf initialized (=0)                */
    fixModembuf = 0;
    /* LA: if (checkmodem) statement below added       */
    if (checkmodem)
        checkModem(); 
...
void formatCID(char *buf)
{
    ...
    if (strstr(buf, "RING"))
    {
        logMsg(LEVEL4, "LA: 000 Detected RING\n");   /* LA: Added */
        /* LA: Added modem reinitialization after RING and NMBR */            
        if (!nomodem)
        {
            /* initialize modem for CID */
            /* but don't checkModem: argument checkmodem = 0 (false) */
            if (doModem(0) < 0) errorExit(-1, ttyport, 0);
        }
        ...
        {
            ++ring;
            sprintf(msgbuf, "LA: 010 sendInfo RING=%d\n", ring); /* LA: Added */
            logMsg(LEVEL4, msgbuf);                              /* LA: Added */
            sendInfo();
        }
        ...
        if (CIDALL3 == cid.status)
        {
            ...
            */
            ...
            cid.status |= CIDNAME;
            logMsg(LEVEL4, "LA: 020 received date, time, nmbr\n"); /* LA: Changed */
        }
        else if (CIDALT3 == (cid.status & IGNHIGH))
        {
            ...
            */
            ...
            cid.status |= CIDNMBR;
            sprintf(msgbuf, "LA: 030 received %s\n", cid.status == CIDALT3 ?
                "date, time, name" : "date, time, name mesg"); /* LA: Changed */
            logMsg(LEVEL4, msgbuf);
        }
        else if (cid.status == (CIDALL3 | CIDMESG))
        {
            ...
            */
            ...
            cid.status |= CIDNAME;
            logMsg(LEVEL4, "LA: 040 received date, time, nmbr, mesg\n"); /* LA: Changed */
        }
        else if (cid.status & (CIDNMBR | CIDNAME))
        {
             ...
             */
            logMsg(LEVEL4, "LA: 050 received nmbr OR name OR both but no date, no time\n"); /* LA: Added */
            if (!(cid.status & CIDNMBR))
            {
                strncpy(cid.cidnmbr, NONMBR, CIDSIZE - 1);
                cid.status |= CIDNMBR;
                logMsg(LEVEL4, "LA: 051 received no nmbr, no date, no time\n"); /* LA: Added */
            }
            else if (!(cid.status & CIDNAME))
            {
                strncpy(cid.cidname, NONAME, CIDSIZE - 1);
                cid.status |= CIDNAME;
                logMsg(LEVEL4, "LA: 052 received no name, no date, no time\n"); /* LA: Added */
            }
        ...
        else if (gencid && cidsent == 0 && ring == 2 )
        {
             ...
             */
            logMsg(LEVEL4, "LA: 060 gencid and RING=2\n"); /* LA: Added */
            ptr = strdate(NOSEP);     /* returns: MMDDYYYY HHMM */
        ...
        else
        {
             ...
             */
            logMsg(LEVEL4, "LA: 070 CID not here yet or already processed\n"); /* LA: Added */
    ...
    if (strncmp(buf, "###", 3) == 0)
    {
         ...
         */
        logMsg(LEVEL4, "LA: 100 Detected NetCallerID or gateway format\n"); /* LA: Changed */
        ...
        if ((ptr = strstr(buf, "NMBR")))
        {
            ...
            else
            {
                ...
                sprintf(msgbuf, "Begin: hittaAlias()\n");
                logMsg(LEVEL4, msgbuf);
                hittaAlias(cid.cidname, cidbuf);
                sprintf(msgbuf, "End:   hittaAlias()\n");
                logMsg(LEVEL4, msgbuf);
                cid.status |= CIDNAME;
                /* LA: Stop  mod: Find Name using hitta.se */
    ...
    else if (strlen(buf) == 70 && isdigit((int) *buf) && *(buf + 9) == '/' && *(buf + 24) == 'M')
    {
        ...
        * No MESG field
        */
        logMsg(LEVEL4, "LA: 200 Detected TCI serial device format\n"); /* LA: Changed */
    ...
    {
        logMsg(LEVEL4, "LA: 300 Detected DATE\n"); /* LA: Added */
    ...
    {
        logMsg(LEVEL4, "LA: 400 Detected TIME\n"); /* LA: Added */
        if ((ptr = strchr(buf, '='))) ++ptr;
        else ptr = buf + 7; /* this should never happen */
        if (*ptr == ' ') ++ptr;
        if (*ptr)
     ...
     */
             ...
             !strncmp(buf, "NBR", 3))
    {
        logMsg(LEVEL4, "LA: 500 Detected NMBR\n"); /* LA: Added */
        /* LA: Added modem reinitialization after RING and NMBR */            
        if (!nomodem)
        {
            /* initialize modem for CID */
        ...
        if (!(cid.status & CIDNMBR))
        {
            ...
            /* LA: Start mod: Find Name using hitta.se & tidy Nmbr */
            sprintf(msgbuf, "LA: 510 Begin: hittaAlias()\n");
            logMsg(LEVEL4, msgbuf);
            hittaAlias(cid.cidname, ptr);
            sprintf(msgbuf, "LA: 520 End:   hittaAlias()\n");
            logMsg(LEVEL4, msgbuf);
            cid.status |= CIDNAME;
            /* LA: Stop mod: Find Name using hitta.se */
    ...
    {
        logMsg(LEVEL4, "LA: 600 Detected NAME\n"); /* LA: Added */
        /*
    ...
    {
        logMsg(LEVEL4, "LA: 700 Detected MESG\n"); /* LA: Added */
        ptr = buf;
        if (*(ptr + 4) == '=') ptr += 5;
        else ptr += 7;
        if (*ptr == ' ') ++ptr;
    ...
    if (cid.status == CIDNODT || cid.status == (CIDNODT | CIDMESG))
    {
        ...
        logMsg(LEVEL4,"LA: 800 No date and time found -- using current\n"); /* LA: Changed */
        ptr = strdate(NOSEP);           /* returns: MMDDYYYY HHMM */
        strncpy(cid.ciddate, ptr, 8);   /*          0123456789012 */
        cid.ciddate[8] = '\0';
        strncpy(cid.cidtime, ptr + 9, 4);
    ...
    if (CIDALL4 == (cid.status & IGNHIGH))
    {
       ...
       sprintf(msgbuf, "LA: 900 received %s\n", cid.status == CIDALL4 ?
           "date, time, nmbr, name" : "date, time, nmbr, name, mesg"); /* LA: Changed */
       logMsg(LEVEL4, msgbuf);
...
{
    char msgbuf[BUFSIZ];                                    /* LA: Added */
    sprintf(msgbuf, "[%s] %s", strdate(WITHSEP), message);  /* LA: Added */
    ...
    {
      //fputs(message, logptr);                             /* LA: Changed */
        fputs(msgbuf, logptr);                              /* LA: Added   */
        fflush(logptr);
    }
}
